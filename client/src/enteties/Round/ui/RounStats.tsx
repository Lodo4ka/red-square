    ...
    <Card className={cn('bg-muted/10', className)}>
      <CardHeader className="flex flex-row items-start gap-3">
        ...
        <div className="flex-1">
          <CardTitle className="text-lg font-semibold">Статистика раунда</CardTitle>
          <CardDescription className="text-base">ID: {round.id}</CardDescription>
        </div>
        <div className={cn('rounded-full px-3 py-1 text-sm font-medium', statusStyles[round.status])}>
          {round.status}
        </div>
      </CardHeader>
